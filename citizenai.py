...
Original file is located at
    https://colab.research.google.com/drive/1bfsCLDS8HQgpGkhOojafWQRIsoA6PasZ
"""
...
try:
    ...
    print("✅ Using IBM Granite model.")
except Exception as e:
    print(f"⚠️ Could not load IBM model, fallback to Mistral. Reason: {e}")
...
def policy_summarizer_v2(text, file):
    try:
        ...
        result = llm(prompt, max_new_tokens=100, return_full_text=False)[0]["generated_text"]
        return result.strip()
    except Exception as e:
...
def kpi_forecasting(csv_file):
    try:
        ...
        return f"⚠️ Error in forecasting: {e}"

# Module 4: Eco Tips Generator
def eco_tips(keyword):
    if not keyword.strip():
        return "⚠️ Please provide a keyword."
...
def detect_anomaly(csv_file):
    try:
        ...
        return f"⚠️ Error in anomaly detection: {e}"
...
with gr.Blocks(theme=custom_theme) as app:
    ...
    gr.Markdown("Built with IBM Granite LLM (or fallback 🤖) to empower urban planning, feedback, sustainability, and innovation.")
